function metodoJacobi(A, b, x0, tol, maxIter) {
    ...
    for (let k = 0; k < maxIter; k++) {
        for (let i = 0; i < n; i++) {
            ...
            xNueva[i] = (b[i] - suma) / A[i][i];
        }

        let maxNum = 0, maxDen = 0;
        for (let i = 0; i < n; i++) {
            let diff = Math.abs(xNueva[i] - x[i]);
            if (diff > maxNum) maxNum = diff;
            if (Math.abs(xNueva[i]) > maxDen) maxDen = Math.abs(xNueva[i]);
        }
        let error = maxDen === 0 ? 0 : maxNum / maxDen;

        // --- MEJORA 3: Salvavidas para evitar divergencia descontrolada ---
        if (isNaN(error) || !isFinite(error)) {
            throw new Error(`El método de Jacobi divergió en la iteración ${k + 1}. Comprueba si la matriz es diagonalmente dominante.`);
        }

        // --- MEJORA 1: Copia profunda de x para que la UI pueda armar la tabla ---
        pasos.push({ 
            iter: k + 1, 
            valores: [...xNueva], // Guardamos el estado actual del vector corregido
            error: error 
        });
