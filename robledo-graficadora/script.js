        ...
        options: {
            ...
            scales: {
                x: {
                    type: 'linear',
                    position: 'bottom',
                    grid: { color: colorBordeCSS },
                    ticks: { color: colorTextoCSS }
                },
                y: {
                    grid: { color: colorBordeCSS },
                    ticks: { color: colorTextoCSS }
...
function compararMetodos() {
    ...
    const metodos = [
        { nombre: "Bisección",      fn: () => metodoBiseccion(f, a, b, tol, maxIter) },
        { nombre: "Regula Falsi",   fn: () => metodoRegulaFalsi(f, a, b, tol, maxIter) },
        { nombre: "Newton-Raphson", fn: () => metodoNewton(f, df, a, tol, maxIter) },
        { nombre: "Secante",        fn: () => metodoSecante(f, a, b, tol, maxIter) },
        { nombre: "Punto Fijo",     fn: () => metodoPuntoFijo(f, a, tol, maxIter) }
    ];
    ...
    metodos.forEach(m => {
        ...
        try {
            ...
            resultados.push({ nombre: m.nombre, convergio: false, motivo: e.message });
        }
    });
